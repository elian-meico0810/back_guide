...
class PlanillaDetalleFacturaViewSet(viewsets.ViewSet, PaginationHandlerMixin):
    ...
    @action(detail=False, methods=["GET"], url_path="obtener-planilla-detalles-facturas", name="Obtener planilla detalle facturas")
    @permission_classes([AllowAnonymous])
    def obtener_planilla_detalle_facturas(self, request):
        try:
            # Obtenemos el queryset desde el servicio
            data = self.service.get_all_planilla_detalle_facturas()
